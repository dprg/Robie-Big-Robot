...
*************************************************/
void step(motor *mtr){
 for (int j=0; j<=20; j++) {	// 4.3 * 200 = 860 or 1 rev (~2.4 per deg)
	  ...
	  if ((mtr->type == UNIPOLAR)){   // && (mtr->dirChgFlg != ESTOP)){  // don't move if limitStateFlg is tripped
		  switch(mtr->seqNum){
			case 0  :
...
void chkLimitSwState(motor *mtr){
  ...
  switch(mtr->limitSwType){
    ...
    case NO_SW:                            //  assumes normally open limit switch at both limits
       ...
       break; 
    case OPTO:                                  //  assumes a single opto interrupter type limit switch with 
       if (switchCW == HIGH) {                  //  a disk with slots at both limits
